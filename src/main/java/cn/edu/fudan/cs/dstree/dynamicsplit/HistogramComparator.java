...
public class HistogramComparator {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        String searchFileName = "G:\\dstree_data\\Series_64_1000000.z.search.txt";
        if (args.length >= 1)
            searchFileName = args[0];
        System.out.println("searchFileName = " + searchFileName);

        String fileName = "G:\\dstree_data\\Series_64_1000000.z.txt";
        if (args.length >= 2)
            fileName = args[1];

        System.out.println("fileName = " + fileName);

        String indexPath = "G:\\dstree_data\\Series_64_1000000.z.txt.idx_dyn_pla2_100_1";
        if (args.length >= 3)
            indexPath = args[2];
