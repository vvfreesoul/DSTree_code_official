...
public class IndexBuilder {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        String fileName = "D:\\Academic\\_Publications\\_as_1st_author\\EPLA\\dstree\\data\\Series_64_1000000.z.txt";
        if (args.length > 0)
            fileName = args[0];
...
//        FileUtils.writeStringToFile(new File(xmlFileName), xml.toString());
    ...
    public static Node buildIndex(double[][] tss, int segmentSize, int threshold) throws IOException{
    	Node root = new Node(threshold);
        ...
        //calc the split points by segmentSize
        int tsLength = tss[0].length;
        short[] points = calcPoints(tsLength, segmentSize);
        root.initSegments(points);
        for(int i = 0; i < tss.length; i++){
        	root.insertNoIo(tss[i]);
        }
        return root;
    }
