...
public class IndexExactSearcher {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        String searchFileName = "G:\\dstree_data\\Series_64_1.z.search.txt";
        if (args.length >= 1)
            searchFileName = args[0];

        System.out.println("searchFileName = " + searchFileName);

        String indexPath = "G:\\dstree_data\\Series_64_1000000.z.1.txt.idx_dyn_100_1";
...
//        Node tmp = bsfAnswer.node;
//        while (tmp.parent != null)
//        {
//            System.out.println("tmp.getFileName() = " + tmp.getFileName());
//            tmp = tmp.parent;
//        }
    ...
    public static double distExactSearch(double[] queryTs, Node root) throws IOException {	//returns the nn distance; no IO required
//        leafCount = 0;
//        calcDistCount = 0;
//        int processTerminalCount = 0;
//        approTime.resume();
...
//        approTime.suspend();
        ...
        bsfAnswer.node = approxNode;
//        bsfAnswer.dist = DistUtil.minDistBinary(approxNode.getFileName(), queryTs);
        bsfAnswer.dist = DistUtil.minDist(approxNode.getTss(), queryTs);
...
//                double[][] tss = TimeSeriesFileUtil.readSeriesFromFileAtOnce(fileName);
//                double[][] tss = TimeSeriesFileUtil.readSeriesFromBinaryFileAtOnce(fileName, queryTs.length);
                List<double[]> tss = minPqItem.node.getTss();
//                double[][] tss = TimeSeriesFileUtil.readSeriesFromFile(fileName);
//                ioTime.suspend();
                minPqItem.dist = DistUtil.minDist(tss, queryTs);
