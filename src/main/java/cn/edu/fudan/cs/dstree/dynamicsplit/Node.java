...
public class Node implements Serializable {
    ...
    public void updateTss(double[] timeSeries){
    	tss.add(timeSeries);
    }
    
    public List<double[]> getTss(){
    	return tss;
    }
    
    public void insertNoIo(double[] timeSeries) throws IOException {
        //update statistics dynamically for leaf and branch
        updateStatistics(timeSeries);

        if (isTerminal()) {
//            append(timeSeries);            //append to file first
        	updateTss(timeSeries);
...
/*
            ...
            for (int i = 0; i < nodePoints.length; i++) {
                ...
                totalRangeValue += nodeRangeValue;
            }
*/

            if (threshold == size) {   //do split
//            if (totalRangeValue >= 400) {   //do split
//                System.out.println("totalRangeValue = " + totalRangeValue);
...
//                fileBufferManager.DeleteFile(getFileName());
            ...
            if (splitPolicy.routeToLeft(timeSeries))
                left.insertNoIo(timeSeries);
            else
                right.insertNoIo(timeSeries);
        }
    }
